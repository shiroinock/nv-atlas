...
`gh` コマンドで未着手の Issue を取得し、優先度の高いものを選定します。

```bash
# status:ready ラベル付きの Issue を取得（enrichment 完了済み）
# status:in-progress の Issue は自動的に除外される（status:ready のみ取得するため）
gh issue list --state open --label "status:ready" --limit 10 --json number,title,labels,body
```

選定基準:
- `status:ready` ラベル付きの Issue のみが対象（`issue-enrichment` スキルで事前に詳細化済み）
- `status:in-progress` ラベル付きの Issue は対象外（他セッションが実装中）
- `priority:high` ラベルがあるものを優先
- ラベルがない場合は番号が小さい（古い）ものを優先
- ユーザーに選定結果を提示し、確認を取る

> **Note**: `status:ready` の Issue がない場合は、先に `issue-enrichment` スキルで Issue を詳細化するよう提案してください。

### 1.5. 排他制御ラベルの付け替え

Issue 選定・ユーザー確認後、実装開始前に `status:ready` → `status:in-progress` にラベルを付け替えます。これにより、他セッションが同じ Issue を重複して着手することを防ぎます。

```bash
gh issue edit {番号} --remove-label "status:ready" --add-label "status:in-progress"
```

> **Note**: `status:in-progress` ラベルが GitHub 上に存在しない場合は、事前に作成してください:
> ```bash
> gh label create "status:in-progress" --description "実装中（排他制御用）" --color "FBCA04"
> ```
...
# 正常完了時: status:in-progress を除去（PR マージ時に Issue クローズされる想定）
...
パイプライン途中でエラーが発生し、リトライ上限に達して中断する場合は、`status:in-progress` → `status:ready` に戻して他セッションが再着手できるようにします。

```bash
# エラー中断時: status:ready に戻す
gh issue edit {番号} --remove-label "status:in-progress" --add-label "status:ready"
```
...
→ リトライ上限で中断する場合は `status:in-progress` → `status:ready` にラベルをロールバック
