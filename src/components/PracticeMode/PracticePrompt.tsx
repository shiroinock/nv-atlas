import type { VimCommand, PracticeScore, KeyInputSpec } from "../../types/vim";
...
export function PracticePrompt({ command, inputSpec, score, lastResult, started, onStart }: PracticePromptProps) {
  if (!started) {
    return (
  ...
  if (!command) {
    return (
      <div className={styles.panel}>
        <p className={styles.message}>選択したカテゴリに出題可能なコマンドがありません</p>
      </div>
    );
  }
  ...
  const showHint = inputSpec && (inputSpec.requiresShift || inputSpec.layerInfo);

  return (
    <div className={`${styles.panel} ${resultClass}`}>
      <div className={styles.prompt}>
        <span className={styles.badge} style={{ backgroundColor: `${color}33`, color }}>
      ...
      <div className={styles.hintRow}>
        {showHint && (
          <span className={styles.hint}>{inputSpec.hint}</span>
        )}
